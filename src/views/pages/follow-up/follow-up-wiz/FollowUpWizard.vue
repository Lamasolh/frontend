...
import FormWizardNumber from '@/views/pages/follow-up/follow-up-wiz/FormWizardNumber.vue'
import 'vue-form-wizard/dist/vue-form-wizard.min.css'
// eslint-disable-next-line import/order
import { BButton } from 'bootstrap-vue'
import FollowUpHeader from '@/views/pages/follow-up/follow-up-wiz/FollowUpHeader.vue'
...
export default {
  ...
  computed: {
    ...
    showNote: takeState('followUpWiz', 'showNote').commit('followUpWiz/SET_SHOW_NOTE').map(),

    phoneissue: takeState('followUpWiz', 'phoneissue').commit('followUpWiz/SET_PHN_ISS').map(),
    referral: takeState('followUpWiz', 'referral').map(),
    governorate: takeState('followUpWiz', 'governorate').map(),
    district: takeState('followUpWiz', 'district').map(),
    cadaster: takeState('followUpWiz', 'cadaster').map(),
    type: takeState('followUpWiz', 'type').map(),
    iss: takeState('followUpWiz', 'iss').map(),
  },
  mounted() {
    this.show1 = false
    this.selected = null
    ...
    this.$store.dispatch('followUpWiz/GET_CASE_FOLL',
      {
      ...
      this.typeTitle = this.$store.getters['followUpWiz/typeOptions'].filter(
        it => it.value === value[0][0].satl_type,
      )[0].title
      this.cadasterTitle = this.$store.getters['followUpWiz/cadOptions'].filter(
        it => it.value === value[0][0].cadastre_id,
      )[0].title
      if (value[0][0].is_id != null) {
        this.issTitle = this.$store.getters['followUpWiz/issOptions'].filter(

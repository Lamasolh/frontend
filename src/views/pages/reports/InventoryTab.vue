...
export default {
  ...
  computed: {
    ...
    invExportField: takeState('report', 'invExportField').map(),
    invField: takeState('report', 'invField').commit('report/SET_INV_FIELD').map(),
    invItem: takeState('report', 'invItem').commit('report/SET_INV_ITEM').map(),
    ExportTable() {
  ...
  methods: {
    ...
    GetInvRep() {
      this.$store.dispatch('report/GET_REPORT_INV', {

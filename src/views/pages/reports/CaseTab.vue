...
export default {
  ...
  computed: {
    ...
    range_rep_case: takeState('report', 'range_rep_case').commit('report/SET_RNG_CASE').map(),

    caseField: takeState('report', 'caseField').map(),
    caseItem: takeState('report', 'caseItem').map(),
    caseExportField: takeState('report', 'caseExportField').map(),
    ExportTable() {
  ...
  methods: {
    ...
    GetCaseRep() {
      ...
      if (this.range_rep_case != null) {
        this.$store.dispatch('report/GET_REPORT_CASE', {

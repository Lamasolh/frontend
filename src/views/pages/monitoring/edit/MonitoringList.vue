<template>
  ...
  >
    ...
    <p class="text-success ml-2">
      Green Color is this case is monitored
    </p>
    <p class="text-info ml-2">
      Blue Color is this case is in progress
    </p>
    <b-table
      ref="selectableTable"
      hover
    ...
    >
      ...
      <template #cell(case_status)="data">
        <b-badge
          pill
          :variant=" $store.getters['monitoring/CaseStatusOptions'].filter(
      ...
      <template
        #cell(awareness_status)="data"
      >
        <b-badge
          pill
          :variant="data.value ===0 ? 'danger' :data.value ===2 ? 'success': 'info'"
        >
          {{
            data.value ===0 ? 'Not Awarened' :data.value ===2 ? 'Awarened': 'Pending'
          }}
        </b-badge>
      </template>
...
export default {
  ...
  computed: {
    ...
    rangeDate: takeState('monitoring', 'rangedateME').commit('monitoring/SET_RANG_DAT_ME').map(),
    ...
    fields: takeState('monitoring', 'caseField').commit('monitoring/SET_FIELD').map(),
    // Table Option
    perPage: takeState('monitoring', 'perPage').commit('monitoring/SET_PER_PAGE').map(),
    pageOptions: takeState('monitoring', 'pageOptions').map(),
    currentPage: takeState('monitoring', 'currentPage').commit('monitoring/SET_CUR_PAGE').map(),
    ...
    showEnd: takeState('monitoring', 'showEnd').commit('monitoring/SET_SHOW_END').map(),
    endItem: takeState('monitoring', 'endItem').commit('monitoring/SET_END_ITEM').map(),
    sortOptions() {
    ...
    itemTable() {
      ...
      const dlv = iss.filter(itemdlv => (this.case_status == null ? itemdlv : itemdlv.case_status === this.case_status.title))
      const feed = dlv.filter(itemfeek => (this.feedback == null ? itemfeek : itemfeek.case_feedback === this.feedback.title))
      const awrs = feed.filter(itemawrs => (this.awareness == null ? itemawrs : itemawrs.awareness_status === this.awareness.value))
      const mnt = awrs.filter(itemmnt => (this.monitoring == null ? itemmnt : itemmnt.monitoring_done === this.monitoring.value))
  ...
  mounted() {
    ...
    this.items = []
    this.$store.dispatch('monitoring/GET_M_E', {
      ...
      if (res.length > 0) {
        this.$swal({
          title: 'M & E Tools',
          text: 'You have an old M&E Operation list, do you want to clear unprogressed cases?',
          icon: 'info',
  ...
  methods: {
    optionClicked(text, icon, item, variant, type) {
      ...
      if (type === 0) {
        ...
        this.$swal({
          ...
          if (result.value) {
            this.show1 = false
            this.$store.dispatch('monitoring/ReviveCase', {
              prefix: this.$route.params.prefixPrj,
              prefixRole: this.$route.params.prefixRole,
              items: item,
              type: this.$route.params.assmnttype,
    ...
    RightClick(item, index, event) {
      ...
      switch (str) {
        ...
        case 'Terminated':
          this.menuData = [
            {
              icon: 'TrendingUpIcon', text: `Revive (${item.case_code})`, variant: 'primary', item, type: 1,
            },
          ]
        ...
        case 'Task Manager':
          this.menuData = [
            {
              icon: 'XIcon', text: `End Case (${item.case_code})`, variant: 'danger', item, type: 0,
            },
          ]
          event.preventDefault()
          this.$refs.menu.open(event, { index, item })
    ...
    SearchME() {
      const rg = this.rangeDate
      this.$store.dispatch('monitoringTools/GET_DATA_ME', {
        prefix: this.$route.params.prefixPrj,
        prefixRole: this.$route.params.prefixRole,
        type: this.$route.params.assmnttype,
        startDate: rg.split('to')[0],

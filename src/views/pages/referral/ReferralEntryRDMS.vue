              ...
              <b-card-footer>
                ...
                >
                  <b-col cols="auto">
                    ...
                    <download-excel
                      v-show="items_blc.filter(e=> e.enable === false).length>0"
...
export default {
  ...
  computed: {
    ...
    // Selected
    items_blc: takeState('referral', 'items_blc').commit('referral/ADD_ITEMS_IMPORT_BLC').map(),
    itemField_blc: takeState('referral', 'itemField_blc').map(),
    json_fields: takeState('referral', 'json_fields').map(),
    ExportTable() {
      console.log(this.items_blc.filter(e => e.enable === false))
      const it = JSON.parse(JSON.stringify(this.items_blc.filter(e => e.enable === false)))
      console.log(it)
      return it
    },
  },
  ...
  methods: {
    SaveData() {
      if (this.items_blc.length > 0) {
        if (this.items_blc.filter(e => e.enable === false).length === 0) {
    ...
    Export(index) {
      switch (index) {
        case 1:
          this.download('/file/excel/Blanked Template.xlsx', 'Blanke Template.xlsx')
          break
        default:
      }
    },
    ...
    ResetData() {
      this.$store.commit('referral/RESET_ITEM_BLC')
    },
  },
}
</script>

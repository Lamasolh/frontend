...
import FormWizardNumber from '@/views/pages/follow-up-conf/follow-up-wiz/FormWizardNumber.vue'
import 'vue-form-wizard/dist/vue-form-wizard.min.css'
// eslint-disable-next-line import/order
import { BButton } from 'bootstrap-vue'
import FollowUpHeader from '@/views/pages/follow-up-conf/follow-up-wiz/FollowUpHeader.vue'
...
export default {
  ...
  computed: {
    ...
    validated: takeState('followUpWizConf', 'validated').map(),
    show: takeState('followUpWizConf', 'show').map(),
    showNote: takeState('followUpWizConf', 'showNote').commit('followUpWizConf/SET_SHOW_NOTE').map(),
    ...
    referral: takeState('followUpWizConf', 'referral').map(),
    governorate: takeState('followUpWizConf', 'governorate').map(),
    district: takeState('followUpWizConf', 'district').map(),
    cadaster: takeState('followUpWizConf', 'cadaster').map(),
    type: takeState('followUpWizConf', 'type').map(),
    iss: takeState('followUpWizConf', 'iss').map(),
  },
  mounted() {
    this.show1 = false
    this.selected = null
    ...
    this.$store.dispatch('followUpWizConf/GET_CASE_FOLL',
      {
      ...
      this.typeTitle = this.$store.getters['followUpWizConf/typeOptions'].filter(
        it => it.value === value[0][0].satl_type,
      )[0].title
      this.cadasterTitle = this.$store.getters['followUpWizConf/cadOptions'].filter(
        it => it.value === value[0][0].cadastre_id,
      )[0].title
      if (value[0][0].is_id != null) {
        this.issTitle = this.$store.getters['followUpWizConf/issOptions'].filter(

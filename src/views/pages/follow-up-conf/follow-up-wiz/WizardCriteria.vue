...
export default {
  ...
  computed: {
    ...
    itemsrec: takeState('followUpWizConf', 'itemsrec').commit('followUpWizConf/SET_ITEM_REC').map(),
    pcrResultOption: takeState('global', 'pcr_result').map(),
    pcres: takeState('followUpWizConf', 'pcres').commit('followUpWizConf/SET_PCR_RES').map(),
    pcrResult() {
  ...
  methods: {
    ...
    refreshImage() {
      ...
      this.pc = null
      this.pc2 = null
      this.$store.dispatch('followUpWizConf/REFRESH_IMAGE')
    },
    dooo(value) {
      this.pcr_photo = null

          ...
          <!--<template #cell(category)="data">
            <div v-if="diSelect">
              <v-select
                ...
                :value=" data.item.role_cat_name"
                :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                label="title"
                :append-to-body="true"
                :clearable="false"
                :options="roleCategory"
                @input="item => ChangeRole(item,data)"
              />
            </div>
            <div v-else>
              <b-overlay
            ...
            <b-row align-h="center">
              <b-col>
                <b-form-checkbox
                  ...
                  :checked="data.value > 0 ? true : false"
                  @change="item=> Input(item,data, data.value )"
                />
              </b-col>
            </b-row>
          </template>
...
export default {
  ...
  computed: {
    ...
    rolesItem: takeState('listOrgPrj', 'rolesItem').commit('listOrgPrj/SET_ITEM').map(),
    roleCategory: takeState('listOrgPrj', 'roleCategory').map(),
    rolesField: takeState('listOrgPrj', 'rolesField').map(),
    rolesItemOriginal: takeState('listOrgPrj', 'rolesItemOriginal').map(),

    selectItem: takeState('listOrgPrj', 'selectItem').commit('listOrgPrj/SET_SELECT').map(),
  },
  mounted() {
    this.ResetData()
  },
  methods: {
    ResetData() {
      this.rolesItem = []
      this.$store.dispatch('listOrgPrj/GET_ROLE', {
    ...
    Save() {
      if (this.selectItem.length > 0) {
        if (this.selectItem[0].user_id != null) {
          this.$store.dispatch('listOrgPrj/Save_ROLE', {
            prefix: this.$route.params.value.proj_prefix,
    ...
    Input(value, item) {
      try {
        ...
        const { user_id } = item.item
        if (this.selectItem.length === 0) {
          this.selectItem.push({
            user_id,
            prefix: key,
            value: (this.rolesItem[item.index][key] === 1) ? 0 : 1,
          })
        } else if ((this.selectItem.filter(it => it.user_id === item.item.user_id && it.prefix === key).length) === 0) {
          this.selectItem.push({
            user_id,
            prefix: key,
            value: this.rolesItem[item.index][key] === 1 ? 0 : 1,
          ...
          this.selectItem.filter(it => it.user_id === this.rolesItem[item.index].user_id)[0].value = value === 1
        }
        console.log('select', this.selectItem)
      } catch (e) {
        console.log('error', e)
      }
    },

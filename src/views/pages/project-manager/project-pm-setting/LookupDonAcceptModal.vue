<template>
  ...
  >
    ...
    <b-container fluid>
      <b-row>
        <b-col>
          <p v-if="showModalDonorAcceptLook.res != null">
            Do You want add this Donner "{{ showModalDonorAcceptLook.res[0].sup_name }}"
          </p>
        </b-col>
      </b-row>
    </b-container>
...
export default {
  ...
  computed: {
    ///
    ...takeState('listPMSetPrj', 'showModalDonorAcceptLook')
      .expose([
        'active',
      ])
      .commit('listPMSetPrj/SET_MOD_DONOR_ACPT_LOOK')
      .map(),
    ...
    accept() {
      this.$store
        .dispatch('listPMSetPrj/SAVE_LOOKUP_SUP_DON', {
          ...
          id: this.showModalDonorAcceptLook.res[0],
          orgType: 3,
        })
        .then(val => {
          console.log('val', val)
          if (val.res.length === 0) {
            ...
            this.$swal({
              title: 'Save Lookup Donor',
              text: 'Donor Al ready Exist',
              icon: 'info',
              customClass: {
                confirmButton: 'btn btn-primary',
              },

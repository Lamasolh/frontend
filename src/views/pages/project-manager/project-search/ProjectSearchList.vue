...
export default {
  ...
  computed: {
    ...
    marker: takeState('listOrgPrj', 'orgMarker').commit('listOrgPrj/SET_MARKER_ITEM').map(),
  ...
  methods: {
    zoomChanged(value) {
      ...
      this.marker.forEach((element, index) => {
        ...
        this.$store.commit('listOrgPrj/REM_ORG_ITEM', index)
        this.$store.commit('listOrgPrj/SET_ORG_ITEM', newMarker)
      })
    },

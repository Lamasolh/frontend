...
export default {
  ...
  computed: {
    show: takeState('reportDRM', 'show').commit('reportDRM/SET_SHOW').map(),
    range_rep_case: takeState('reportDRM', 'range_rep_activity').commit('reportDRM/SET_RNG_ACTIVITY').map(),

    caseField: takeState('reportDRM', 'activityField').map(),
    caseItem: takeState('reportDRM', 'activityItem').map(),
    caseExportField: takeState('reportDRM', 'activityExportField').map(),
  ...
  methods: {
    ...
    GetCaseRep() {
      ...
      if (this.range_rep_case != null) {
        this.$store.dispatch('reportDRM/GET_REPORT_ACTIVITY', {

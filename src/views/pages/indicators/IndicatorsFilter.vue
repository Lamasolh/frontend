...
export default {
  ...
  computed: {
    ...
    cadOptions: takeState('indicators', 'cadOptions').commit('indicators/SET_CAD_OPT').map(),
    typeOptions: takeState('global', 'type').map(),
    issOptions: takeState('indicators', 'issOptions').map(),
    // Filter Values
  ...
  methods: {
    ...
    clearAll() {
      ...
      this.filter = null
      this.filterOn = []
      localStorage.removeItem('indicators:TypeOptions')
      localStorage.removeItem('indicators:CadOptions')
      localStorage.removeItem('indicators:IssOptions')
      localStorage.removeItem('indicators:Referral')
      localStorage.removeItem('indicators:Gov')
      localStorage.removeItem('indicators:Dis')
      localStorage.removeItem('indicators:Type')
      localStorage.removeItem('indicators:Cadaster')
      localStorage.removeItem('indicators:ISS')
      localStorage.removeItem('indicators:RangeDate')
      localStorage.removeItem('indicators:PerPage')
      localStorage.removeItem('indicators:sortBy')
      localStorage.removeItem('indicators:sortDesc')
      localStorage.removeItem('indicators:sortDirection')
      localStorage.removeItem('indicators:filterOn')
    },
    ChangeDistrict(value) {
      if (value != null) {
        if (this.type != null) {
          this.$store.dispatch('indicators/GET_CAD').catch(() => {
            this.$swal({
              title: 'Error!',
    ...
    ChangeCAD(value) {
      if (value != null) {
        this.$store.dispatch('indicators/GET_ISS').catch(() => {
          this.$swal({
            title: 'Error!',
            text: 'Please Check internet and try again',
            icon: 'error',

...
export default {
  ...
  computed: {
    ...
    dataTypeOptions: takeState('referralClr', 'dataTypeOptions').map(),
    yesNo: takeState('referralClr', 'yesNo').map(),
    // Family Info
    ...
    /// MUltifield
    multiField: takeState('referralClr', 'filedOptions').commit('referralClr/ADD_ITEMS_FILED').map(),
  ...
  methods: {
    ...
    removeFiled(index) {
      this.$store.commit('referralClr/REM_ITEMS_IND', { index })
    },
    repeateAgain() {
      this.$store.commit('referralClr/ADD_ITEMS_FILED', {
        id: this.lastId,
        name: null,
        desc: null,

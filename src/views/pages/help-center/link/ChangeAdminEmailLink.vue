      ...
      >
        ...
        >
          ...
          <b-card-text class="mb-2">
            Enter new email to become the new email for your organization admin
          </b-card-text>
...
export default {
  ...
  data() {
    return {
      alert: null,
      sideImg: require('@/assets/images/illustration/api.svg'),
      // validation
    }
  },
  computed: {
    imgUrl() {
      if (store.state.appConfig.layout.skin === 'dark') {
        ...
        this.sideImg = require('@/assets/images/illustration/api.svg')
        return this.sideImg
      }
      return this.sideImg
    },
    show: takeState('helpCenter', 'show').commit('helpCenter/SET_SHOW').map(),
    email: takeState('helpCenter', 'changeAdminEmail').commit('helpCenter/SET_CHANGE_ORG_ADMIN_LINK').map(),
  },
  ...
  mounted() {
    this.$store.dispatch('helpCenter/GET_ChangeAdminEmail', {
  ...
  methods: {
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          this.$store.dispatch('helpCenter/ChangeAdminEmailLink').then(res => {
            if (res === 1) {
              this.$toast({

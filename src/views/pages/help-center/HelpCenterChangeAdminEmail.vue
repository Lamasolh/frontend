    ...
    <validation-observer ref="directForm">
      ...
      >
        ...
        >
          ...
          <b-row>
            <p class="ml-2">
              To change admin email enter organization email and we send a email with link to verify your organization
            </p>
          </b-row>
          <b-row>
            <!-- email -->
...
export default {
  ...
  computed: {
    email: takeState('helpCenter', 'emaiLorg').commit('helpCenter/SET_CHANGE_ORG_ADMIN').map(),
  ...
  methods: {
    Send() {
      this.$refs.directForm.validate().then(success => {
        if (success) {
          this.alert = null
          this.$store.dispatch('helpCenter/ChangeAdminEmail').then(res => {
            console.log(res)
            if (res === 1) {
              this.alert = 1
            } else {

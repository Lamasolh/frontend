    ...
    <validation-observer ref="directForm">
      ...
      >
        ...
        >
          ...
          <b-row>
            <p class="ml-2">
              To change organization admin enter your organization email and we send an email with link to verify your organization            </p>
          </b-row>
          <b-row>
            <!-- email -->
            <b-col cols="12">
...
export default {
  ...
  computed: {
    email: takeState('helpCenter', 'orgadmin').commit('helpCenter/SET_CHANGE_ADMIN').map(),
  ...
  methods: {
    Send() {
      this.$refs.directForm.validate().then(success => {
        if (success) {
          this.alert = null
          this.$store.dispatch('helpCenter/ChangeAdmin').then(res => {
            console.log(res)
            if (res === 1) {
              this.alert = 1
            } else {

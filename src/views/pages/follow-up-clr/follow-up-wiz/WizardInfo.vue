...
export default {
  ...
  computed: {
    yesNo: takeState('followUpWizClr', 'yesNo').map(),

    phoneissue: takeState('followUpWizClr', 'phoneissue').commit('followUpWizClr/SET_PHN_ISS').map(),
    orglistselect: takeState('followUpWizClr', 'orglistselect').commit('followUpWizClr/SET_SELECTD_VAL_ORG').map(),
    ...takeState('followUpWizClr', 'items')
      ...
      .map(),
    items: takeState('followUpWizClr', 'items').map(),
    // Options
    ...
    orgOption: takeState('followUpWizClr', 'orglist').map(),
    ...
    referralOptions: takeState('followUpWizClr', 'referralOptions').map(),
    governorateOption: takeState('global', 'governorate').map(),
    districtOptions: takeState('global', 'district').map(),
    cadOptions: takeState('followUpWizClr', 'cadOptions').map(),
    typeOptions: takeState('followUpWizClr', 'typeOptions').map(),
    issOptions: takeState('followUpWizClr', 'issOptions').map(),
    // Select Value
    referral: takeState('followUpWizClr', 'referral').commit('followUpWizClr/SET_REF').map(),
    governorate: takeState('followUpWizClr', 'governorate').commit('followUpWizClr/SET_GOV').map(),
    district: takeState('followUpWizClr', 'district').commit('followUpWizClr/SET_DIS').map(),
    cadaster: takeState('followUpWizClr', 'cadaster').commit('followUpWizClr/SET_CAD').map(),
    type: takeState('followUpWizClr', 'type').commit('followUpWizClr/SET_TYPE').map(),
    iss: takeState('followUpWizClr', 'iss').commit('followUpWizClr/SET_ISS').map(),
    nationality: takeState('followUpWizClr', 'nationality').commit('followUpWizClr/SET_NAT').map(),
    gender: takeState('followUpWizClr', 'gender').commit('followUpWizClr/SET_GEN').map(),
    country: takeState('followUpWizClr', 'country').commit('followUpWizClr/SET_CON').map(),
    multiField: takeState('followUpWizClr', 'filedOptions').commit('followUpWizClr/ADD_ITEMS_FILED').map(),
  ...
  methods: {
    ...
    munChange() {
      this.$store.dispatch('followUpWizClr/GetOrglist', {
    ...
    ChangeDistrict(value) {
      if (value != null) {
        this.$store.dispatch('followUpWizClr/GET_TYPE')
      }
    },
    ChangeType(value) {
      if (value != null) {
        ...
        if (value.value <= 3) {
          if (this.cadaster != null) {
            ...
            this.cadaster = null
            this.$store.dispatch('followUpWizClr/GET_CAD')
          }
        } else {
          console.log('Is s')
          this.cadaster = null
    ...
    ChangeCAD(value) {
      if (value != null) {
        if (this.type.value !== 1 && this.type.value !== 2 && this.type.value !== 3) {
          this.$store.dispatch('followUpWizClr/GET_ISS')
        } else {
          this.munChange()
        }
      }
    ...
    SameIss() {
      ...
      this.$store.dispatch('followUpWizClr/GET_LAT_LNG').then(resp => {
        console.log(resp)
    ...
    checkDuplicate() {
      this.$store.dispatch('followUpWizClr/CHECK_DUPL', {
        prefix: this.$route.params.prefixPrj,

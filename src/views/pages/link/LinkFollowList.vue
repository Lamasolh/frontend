    ...
    >
      ...
      <template #cell(active)="data">
        <b-badge
          ...
          :variant="data.value ===1 ? 'success' : 'warning'"
        >
          {{ data.value ===0 ? 'Submitted':'Pending' }}
        </b-badge>
      </template>
      <template #cell(referral_date)="data">
        <p class="text-center">
          {{ dur(data) }} {{ dur(data)>1? 'Days':'Day' }}
        </p>
      </template>
...
export default {
  ...
  computed: {
    ...
    // Table Data
    items: takeState('link', 'caseItem').commit('link/SET_ITEM').map(),
    fields: takeState('link', 'caseField').commit('link/SET_FIELD').map(),
    // Table Option
    perPage: takeState('link', 'perPage').commit('link/SET_PER_PAGE').map(),
    pageOptions: takeState('link', 'pageOptions').map(),
    currentPage: takeState('link', 'currentPage').commit('link/SET_CUR_PAGE').map(),
  ...
  mounted() {
    if (this.$store.getters['link/isInView'] === false) {
      // Set the initial number of items
      this.totalRows = this.itemTable.length
    } else {
      /* const items = this.$store.getters['link/items']
  ...
  methods: {
    showInMap(value) {
      if (value.item.address_latitude !== 0 || value.item.address_longitude !== 0) {
        this.$store.commit('link/SET_MAP_CENTER', {
    ...
    dur(dd) {
      return moment(moment()).diff(dd.value, 'days')
    },
    refresh() {

...
export default {
  ...
  computed: {
    ...
    range_rep_case: takeState('reportsTools', 'range_rep_activity').commit('reportsTools/SET_RNG_ACTIVITY').map(),

    caseField: takeState('reportsTools', 'activityField').map(),
    caseItem: takeState('reportsTools', 'activityItem').map(),
    caseExportField: takeState('reportsTools', 'activityExportField').map(),
  ...
  methods: {
    ...
    GetCaseRep() {
      ...
      if (this.range_rep_case != null) {
        this.$store.dispatch('reportsTools/GET_REPORT_ACTIVITY', {

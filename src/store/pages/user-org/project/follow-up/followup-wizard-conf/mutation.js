...
export default {
  ...
  SET_ITEM(state, { key, value }) {
    ...
    store.commit('followUpWizConf/CALC_SCORE')
  },
    ...
    state.filedOptions.forEach(el => {
      if (JSON.parse(family.extra_fields) != null) {
        if (
          ...
          state.filedOptions.filter(f => f.id === el.id)[0].value = JSON.parse(family.extra_fields).filter(
            e => e[0] === el.id,
          )[0][1]
        }
      }
  ...
  SET_REF(state, payload) {
    ...
    store.commit('followUpWizConf/SET_ITEM', {
      key: 'referral_id',
      value: payload.value,
    })
  },
  ...
  SET_MUN(state, payload) {
    ...
    store.commit('followUpWizConf/MUN_CHNG')
  },
  ...
  SET_GEN(state, payload) {
    ...
    store.commit('followUpWizConf/SET_ITEM_FAM', {
      key: 'gender',
      value: payload.value,
    })
  },
  ...
  /* SET_PHN_ISS(state, payload) {
    ...
    store.commit('followUpWizConf/phoneissue', { value: payload.title }
    //state.phoneissue = payload.title
  },
  // store.commit('followUpWizConf/SET_ITEM', { key: 'priority_level', value: payload.value } */
  SET_PHN_ISS(state, payload) {
    state.phoneissue = payload.title
  },
  ...
  SET_PCR_RES(state, payload) {
    state.pcres = payload
    store.commit('followUpWizConf/SET_ITEM_REC', {
      key: 'pcr_result',
      value: payload.value,
    })
  },
  ...
  ADD_IND_FILED(state, payload) {
    ...
    payload
      ...
      .forEach(el => {
        ...
        state.filedOptions.push({
          id: el.id,
          isInd: JSON.parse(el.in_pages.toString().replace(/\\/g, ''))
            .isInd ?? false,
          inPages: JSON.parse(

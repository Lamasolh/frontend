...
export default {
  SAVE_DATA({ state }, payload) {
    ...
    return new Promise((resolve, reject) => {
      API.apiClient
        .post('/api/project/referral-conf', {
        ...
        .then(resp => {
          ...
          resp.data.result.forEach((el, index) => {
            ...
            console.log('////////////////////')
            console.log(el[0], el[1], el[0].result)
            console.log('////////////////////')
            console.log('El:', state.items)
            if (el[0].result === 1) {
              state.items.filter(
                it => it.id === el[1].id,
              ...
              console.log(`Res1: Case Added- ${el[0].case_code}`)
            ...
            .forEach((el, index) => {
              ...
              if (el.id === dt[index][1].id) {
                if (dt[index][0].result === 1) {
                  ...
                  state.items.filter(
                    it => it.id === dt[index][1].id,
                  )[0].code = dt[index][0].case_code
                  state.items[index].enable = false
                } else if (dt[index][0].result === 2) {
                  state.items.filter(
                    it => it.id === dt[index][1].id,
                  )[0]._rowVariant = 'danger'
                  state.items.filter(
                    it => it.id === dt[index][1].id,
                  )[0].code = `Case Duplication: ${dt[index][0].case_code}`
                  console.log(
                    state.items.filter(
                      it => it.id === dt[index][1].id,
                    ),
  ...
  ADD_ITEMS_IMPORT({ state, commit, rootState }, payload) {
    ...
    return new Promise((resolve, reject) => {
      API.apiClient
        .post('/api/project/check_import_clr', {
          items: payload.items,
          type: payload.type,
        })
        .then(resp => {

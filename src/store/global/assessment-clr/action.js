...
export default {
  ...
  FETCH(context, { resp }) {
    ...
    if (context.getters.orgOptions.length === 0) {
      context.commit('ADD_ORG', { data: resp.orgop })
    }
    ...
    if (context.rootGetters['global/pcr_result'].length === 0) {
      context.commit('global/ADD_ALL_PCR_RES', { data: resp.pcr_res }, { root: true })
    }

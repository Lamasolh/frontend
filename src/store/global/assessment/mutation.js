...
export default {
  SET_CON(state, { key, value }) {
    ...
    store.commit('assessment/SET_ITEM', { key: 'case_phone', value: phone })
    store.dispatch('assessment/SaveLocalStorage')
  },
  CALC_SCORE(state) {
    let scr = 0
  ...
  SET_ITEM(state, { key, value }) {
    ...
    store.commit('assessment/CALC_SCORE')
  },
  ...
  SET_SELECT(state, { items, family, getters }) {
    try {
      ...
      state.governorate = getters['assessment/governorateOption'].filter(
        it => it.value === items.governorate_id,
      ).shift()
      state.district = getters['assessment/disOption'].filter(
        it => it.value === items.district_id,
      ).shift()
      state.type = getters['assessment/typeOptions'].filter(
        it => it.value === items.satl_type,
      ).shift()
      state.cadaster = getters['assessment/cadOptions'].filter(
        it => it.value === items.cadastre_id,
      ).shift()
      if (items.is_id == null) {
        state.iss = null
      } else {
        state.iss = getters['assessment/issOptions'].filter(
  ...
  SET_REF(state, payload) {
    ...
    store.commit('assessment/SET_ITEM', { key: 'referral_id', value: payload.value })
  },
  SET_GOV(state, payload) {
    ...
    state.cadOptions = []
    state.issOptions = []
    store.commit('assessment/SET_ITEM', { key: 'governorate_id', value: payload.value })
  },
  SET_DIS(state, payload) {
    ...
    state.issOptions = []
    state.typeOptions = []
    store.commit('assessment/SET_ITEM', { key: 'district_id', value: payload.value })
  },
  SET_TYPE(state, payload) {
    ...
    state.iss = null
    state.saved = true
    store.commit('assessment/SET_ITEM', { key: 'satl_type', value: payload.value })
  },
  SET_CAD(state, payload) {
    ...
    store.commit('assessment/SET_ITEM', { key: 'cadastre_id', value: payload.value })
  },
  SET_ISS(state, payload) {
    state.iss = payload
    state.munChange = true
    store.commit('assessment/SET_ITEM', { key: 'is_id', value: payload.value })
  },
  SET_NAT(state, payload) {
    state.nationality = payload
    store.commit('assessment/SET_ITEM', { key: 'nationality_id', value: payload.value })
  },
  SET_GEN(state, payload) {
    state.gender = payload
    store.commit('assessment/SET_ITEM_FAM', { key: 'gender', value: payload.value })
  },
  SET_EMPL(state, payload) {
    state.employment = payload
    if (payload.value === 1) {
      store.commit('assessment/SET_ITEM', { key: 'empl_type', value: 0 })
    } else {
      store.commit('assessment/SET_ITEM', { key: 'empl_type', value: 1 })
    }
    store.commit('assessment/SET_ITEM', { key: 'empl_type', value: payload.value })
  },
  SET_GVR(state, payload) {
    state.caregiver = payload
    store.commit('assessment/SET_ITEM', { key: 'caregiver', value: payload.value })
  },
